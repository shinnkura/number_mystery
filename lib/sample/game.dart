...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: GamePage(
          title: 'Color match', difficulty: 1, colors: 4, name: "John Doe"),
    );
  }
}
...
class GamePage extends StatefulWidget {
  ...
  GamePage({Key? key, this.title, this.difficulty, this.colors, this.name})
      : super(key: key);
...
class _MyHomePageState extends State<GamePage>
  ...
  int dfs(int i, int? j, int? color, List<dynamic> li) {
    if (li[i][j] == color) {
      int val = 1;
      li[i][j] = -1;
      if (i - 1 >= 0 && li[i - 1][j] != -1) val += dfs(i - 1, j, color, li);
      if (i + 1 <= 9 && li[i + 1][j] != -1) val += dfs(i + 1, j, color, li);
      if (j! - 1 >= 0 && li[i][j - 1] != -1) val += dfs(i, j - 1, color, li);
      if (j + 1 <= 9 && li[i][j + 1] != -1) val += dfs(i, j + 1, color, li);
      return val;
    }
    return 0;
  }
  ...
  update(int? i, int? j) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (movesleft == true) break;
    }
    bool over = false;
    if (movesleft == false) over = true;
  ...
  /* Minimax function */
  List<int?> calculateDP(List<dynamic>? li, bool maxi, int level, int lvlScore,
      int? alpha, int? beta) {
    if (level == 0) {
      return [lvlScore, -1, -1];
    }
        ...
        for (var key in keys) {
          for (var value in dic[key]) {
            ...
            if (temp[value[0]][value[1]] != -1) {
              ...
              var ret = calculateDP(temp, !maxi, level - 1,
                  lvlScore + scoreToAdd as int, alpha, beta);
              if (v! < ret[0]!) {
                v = ret[0];
                valI = value[0];
                valJ = value[1];
          ...
          body: Column(children: <Widget>[
            ...
            Padding(
                ...
                child: IntrinsicHeight(
                  ...
                  Container(
                    child: Image(
                        image: AssetImage('assets/images/undraw_computer.png')),
                    height: (MediaQuery.of(context).size.height -
                            appBar.preferredSize.height) /
                        8,
                    width: (MediaQuery.of(context).size.width -
